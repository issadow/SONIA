...
import static icaro.aplicaciones.recursos.comuSONIAChat.imp.util.ConexionSONIA.VERSION;
 ...
 * @author FGarijo
 */
public class InterpreteMsgsSONIA {
	...
	private ConexionSONIA conectorSONIA;
	...
	public InterpreteMsgsSONIA () {
	}

	public InterpreteMsgsSONIA (ConexionSONIA conexSONIA) {
		conectorSONIA = conexSONIA;
	}

	public synchronized void setConectorSONIA(ConexionSONIA SONIAConect) {
		conectorSONIA = SONIAConect;
	}
	...
	protected void onPrivateMessage(String textoUsuario) {
		...
		if (itfUsoExtractorSem != null) {
			try {
				...
				Logger.getLogger(InterpreteMsgsSONIA.class.getName()).log(
	...
	private ArrayList interpretarAnotaciones(String interlocutor,
		 ...
		 * veces, es un fix r醦ido que deber韆 ser corregido en el Annie
		 * Transducer, cosa que no tengo muy claro c髆o hacerlo...
		 */
