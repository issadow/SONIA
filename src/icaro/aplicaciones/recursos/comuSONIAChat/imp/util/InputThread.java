...
public class InputThread extends Thread {
	 ...
	 * @param bwriter
	 */
	private InterpreteMsgsSONIA  _interpreteMensajes = null;
	private ConexionSONIA _conexionG = null;
	private SONIAUserChat _SONIAGUI = null;

	public static final int MAX_LINE_LENGTH = 510;

	protected InputThread(ConexionSONIA con, InterpreteMsgsSONIA intrpmsg, SONIAUserChat gui) {
	...
	public void run() {
		...
		try {
			while (running) {
				try {
					String line = null;
					while ((line = _SONIAGUI.nextLine()) != null) {
						try {
